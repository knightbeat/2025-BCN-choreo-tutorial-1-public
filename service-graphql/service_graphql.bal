...
service /roomgql on new graphql:Listener(9090) {
    ...
    resource function get roomTypes(string checkinDate, string checkoutDate, int guestCapacity) returns RoomType[]|error {
        log:printInfo("searching for description:", checkinDate = checkinDate, checkoutDate = checkoutDate, guestCapacity = guestCapacity);
    ...
    resource function get avaiableRooms(string checkinDate, string checkoutDate, int guestCapacity) returns RoomData[]|error {
        log:printInfo("searching for description:", checkinDate = checkinDate, checkoutDate = checkoutDate, guestCapacity = guestCapacity);
        RoomData[]|error availableRooms = getAvailableRooms(checkinDate, checkoutDate, guestCapacity);
